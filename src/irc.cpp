...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ZSEcoinTEST2\r");
            Send(hSocket, "WHO #ZSEcoinTEST2\r");
        } else {
            // randomly join #ZSEcoin00-#ZSEcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ZSEcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ZSEcoin%02d\r", channel_number).c_str());
        }
